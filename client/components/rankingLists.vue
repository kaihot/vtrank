<template>
    <!--<v-flex>-->
        <v-container>
                <nuxt-link v-for="(item, index) in lists" :key="index" :to='{name:"channel-id",params:{id:item.channel_id}}'  style="text-decoration: none;">
            <v-layout row wrap align-baseline justify-center>
                    ...
                    <v-flex xs6 offset-xs6 sm3 md4 lg3>
                ...
                <v-layout >
                    <v-flex xs12 text-xs-center>
                        <v-pagination :length="this.rankingMeta.last_page" v-model="rankingMeta.current_page" :total-visible="5" @input="handlePaginate"></v-pagination>
                    </v-flex>
                </v-layout>
        </v-container>
    <!--</v-flex>-->
...
<script>
    export default {
        name: "ranking-lists",
        props:["lists", "pageNum", "rankingMeta"],
        data(){
          return{
              page:this.pageNum,
          }
